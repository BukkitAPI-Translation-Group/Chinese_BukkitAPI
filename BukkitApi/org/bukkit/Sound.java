 ...
 * 服务器能给玩家发送的声音的列表.
 * <p>
 * 此接口列出的声音值存在于默认服务器中, 或可以通过 {@link FeatureFlag} 启用.
 * 服务器可能存在其它声音 (比如来自{@link DataPack 数据包}的声音),
 * 这些声音可通过 {@link Registry#SOUNDS} 访问.
 * <p>
 * 警告:在某些时候, 声音可能被这个列表或 Minecraft 自身新增/移除!
 * 不保证声音能播放, 不保证一些声音值会从这个列表移除.
 * 因此, 您不应该依赖这个类的序数值.
 */
